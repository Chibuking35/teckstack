...
const TeamMembers = () => {
  ...
  return (
    <div className="w-full mt-10 md:mt-20">
      <div className="relative h-96 md:h-[600px] overflow-visible flex items-center justify-center">
        ...
        <div className="relative z-20 w-[80%] h-[80%] flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex py-4 px-4 text-center z-30">
            ...
            <div className="flex-1 flex flex-col items-start p-4">
              ...
              <div className="max-w-xl mt-10 rounded-xl shadow-lg">
                <div className="text-justify">
                  {activeTab === "education" && (
                    <p className="mt-2 text-xs font-thin text-gray-300">{member.education}</p>
                  )}
                  {activeTab === "bio" && (
                    <p className="mt-2 text-xs font-thin text-gray-300">{member.bio}</p>
                  )}
                  {activeTab === "skills" && (
                    <div>
          ...
          {/* Profile Picture */}
          <div className="absolute -bottom-6 left-18 z-40">
            <Image src={member.profileImg} alt="profile" width={350} height={350} className="object-cover" />
          </div>

          {/* Navigation Buttons */}
          <div className="absolute top-80 -right-4 -translate-y-1/2 z-50 flex flex-col">
            <button
            ...
            <button
              className="p-2 bg-amber-400 shadow-lg hover:bg-amber-500"
              onClick={() => setActiveMemberIndex((prev) => (prev + 1) % team.length)}
            >
              <FaArrowRight />
            </button>
          </div>
